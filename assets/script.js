...
function checkout() {
    ...
    orderStr += '<p style="border-top: 1px #000 solid; border-bottom: 1px #000 solid; line-height:2em;">' + "以下為領取人（您）的聯絡資料" + '</p>' + '<p>' + (d.filter(function(i) {
        ...
        return i.id == department
    })[0].name : '') + typeObj[type] + gradeObj[grade] + '<br />' + '<span style="color: #00F; font-weight: 1000; font-size: 1.5em;">' + name + '</span>' + '</p>' + '<p>' + '學號：<span style="font-weight: 1000;">' + studentID + '</span>' + '<br />' + '手機：<span style="font-weight: 1000;">' + phoneNumber + '</span>' + '<br />' + '信箱：<span style="font-weight: 1000;">' + email + '</span>' + '<br />' + '取貨日期：<span style="font-weight: 1000; background-color: #FF0;">' + moment(new Date(availableDate.replace(/-/g, '/'))).format('M[月]D[日]') + '（' + week[(new Date(availableDate.replace(/-/g, '/'))).getDay()] + '）' + '</span>' + '<br />' + '取貨時間：<span style="font-weight: 1000; background-color: #FF0;">' + period + '</span>' + '<br />' + '取貨地點：<span style="font-weight: 1000; background-color: #FF0;">' + pickLocation + '</span>' + '<br />' + '（時間地點如有變更將在粉絲專頁公布）' + '</p>';
